...
import backup from '../assets/no_image.webp'
import { convertMinutes, formatCurrency } from "../utils/utils";
    ...
    <main className="container">
      ...
      <div className="row">
        <div className="col-md-4">
          <img src={image} className="img-fluid img-thumbnail img_details"/>
        </div>
          ...
          {movie.genres?(
            <p className="d-flex gap-3">
              {movie.genres.map((genre) => (
                <span key={genre.id} className="badge bg-danger px-3 py-1">{genre.name}</span>
              ))}
            </p>
            ) : (
             ""
            ...
            <p className="mt-2">
              <i className="bi bi-star-fill text-warning"></i> {movie.vote_average} | <i className="bi 
              bi-people-fill text-success"></i> {movie.vote_count} reviews
            </p>
